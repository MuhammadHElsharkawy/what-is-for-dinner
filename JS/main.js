class RecipeApp {
    #state = {
        ...
        currentTab: 'ingredients-tab',
        isMobileMenuOpen: false
    };
    #dom = {};
    #recipes = [];

    constructor(recipes, selectors) {
        this.#recipes = recipes;

        Object.entries(selectors).forEach(([key, selector]) => {
            if (selector.startsWith('all:')) {
                const pureSelector = selector.replace('all:', '');
                const elements = document.querySelectorAll(pureSelector);
                if (elements.length === 0) {
                    throw new Error(`Required elements for selector "${pureSelector}" not found`);
                }
                this.#dom[key] = elements;
            } else {
    ...
    init() {
        ...
        if (this.#recipes.length > 0) {
            this.#state.currentRecipeIndex = this.#getUniqueRandomIndex();
        }
        this.#displayRecipe();
    }
    ...
    #attachEventListeners() {
        ...
        this.#dom.tryAnotherBtn.addEventListener('click', () => this.#showRandomRecipe());
    ...
    #handleTabClick(event) {
        this.#state.currentTab = event.currentTarget.dataset.tab;
        this.#updateTabUI();
    }

    #updateTabUI() {
        this.#dom.tabContents.forEach(tab => {
            const isActive = tab.id === this.#state.currentTab;
    ...
    #getUniqueRandomIndex() {
        let newIndex;
        do {
            newIndex = this.#getRandomInt(this.#recipes.length);
        } while (newIndex === this.#state.currentRecipeIndex);

        return newIndex;
    }

    #displayRecipe() {
        const recipe = this.#recipes[this.#state.currentRecipeIndex];
        if (!recipe) throw new Error('Recipe not found');

        this.#displayImageSection(recipe);
        this.#displayHeaderSection(recipe);
        this.#displayTimeWarning(recipe);
        this.#displayIngredients(recipe);
        this.#displayInstructions(recipe);
        this.#displayNutrition(recipe);
        this.#displayTips(recipe);
    }

    #displayImageSection(recipe) {
        this.#dom.recipeImage.src = recipe.imgCover;
        this.#dom.recipeImage.alt = recipe.imageAlt;
        this.#dom.recipeRate.textContent = recipe.ratingsAverage;
        this.#dom.recipeReviews.textContent = `(${recipe.ratingsQuantity} reviews)`;
        this.#dom.prepTime.textContent = recipe.prepTime;
        this.#dom.cookTime.textContent = recipe.cookTime;
        this.#dom.servings.textContent = recipe.servings;
    }

    #displayHeaderSection(recipe) {
        this.#dom.recipeName.textContent = recipe.name;
        this.#dom.recipeDescription.textContent = recipe.description;
        this.#dom.country.textContent = recipe.category;
        this.#dom.difficulty.textContent = recipe.difficulty;
    }

    #displayTimeWarning(recipe) {
        this.#dom.timeWarning.classList.toggle('hidden', recipe.totalTime < 45);
    }
    ...
    #displayInstructions(recipe) {
        this.#dom.instructionsList.innerHTML = '';
        recipe.instructions.forEach((instruction, index) => {
            const div = document.createElement('div');
            div.className = 'flex items-center gap-4';
            div.innerHTML = `
                <div class="w-12 h-12 bg-orange-500 rounded-2xl flex items-center justify-center text-white text-xl font-bold shrink-0">
                    ${index + 1}
                </div>
            `;
            const contentDiv = document.createElement('div');
            contentDiv.className = 'flex-1 pt-2';
            const p = document.createElement('p');
            p.className = 'text-gray-700';
            p.textContent = instruction;
            contentDiv.appendChild(p);
            div.appendChild(contentDiv);
            this.#dom.instructionsList.appendChild(div);
        });
    }

    #displayTips(recipe) {
        this.#dom.tipsList.innerHTML = '';
        recipe.tips.forEach((tip, index) => {
            const div = document.createElement('div');
            div.className = 'flex items-center gap-3 p-4 bg-amber-50 rounded-xl border-l-4 border-amber-400';
            div.innerHTML = `
                <i class="text-amber-600 text-xl fa-solid fa-circle-check"></i>
            `;
            const p = document.createElement('p');
            p.className = 'text-gray-700';
            p.textContent = tip;
            div.appendChild(p);
            this.#dom.tipsList.appendChild(div);
        });
    }

    #displayNutrition(recipe) {
        this.#dom.caloriesValue.textContent = recipe.nutrition.calories;
        this.#dom.proteinValue.textContent = recipe.nutrition.protein;
        this.#dom.carbsValue.textContent = recipe.nutrition.carbs;
        this.#dom.fatValue.textContent = recipe.nutrition.fat;
        this.#dom.fiberValue.textContent = recipe.nutrition.fiber;
        this.#dom.sodiumValue.textContent = recipe.nutrition.sodium;
    }
}
...
const recipes = [
    {
        ...
        imageAlt: "gourmet spaghetti carbonara pasta dish, professional food photography, appetizing",
        ...
        imageAlt: "honey garlic salmon with vegetables, gourmet presentation, food photography",
        ...
        imageAlt: "thai green curry with vegetables, vibrant and aromatic, asian cuisine, food photography",
        ...
        imageAlt: "gourmet beef burger with lettuce tomato cheese, professional food photography",
        ...
        imageAlt: "mediterranean quinoa bowl with vegetables and tahini, healthy food, colorful presentation",
        ...
        imageAlt: "chicken tikka masala curry with rice, indian food, professional photography",
        ...
        imageAlt: "margherita pizza with fresh basil, italian food, food photography",
        ...
        imageAlt: "beef tacos with toppings, mexican street food, food photography",
        ...
        imageAlt: "caesar salad with parmesan and croutons, fresh salad, food photography",
        ...
        imageAlt: "pad thai noodles with shrimp, thai street food, food photography",
        ...
        imageAlt: "french onion soup with cheese, comfort food, food photography",
        ...
        imageAlt: "chicken stir fry with vegetables, asian cuisine, food photography",
        ...
        imageAlt: "lasagna bolognese layers, italian comfort food, food photography",
        ...
        imageAlt: "shrimp scampi with pasta, seafood, food photography",
        ...
        imageAlt: "vegetable curry with rice, vegan food, food photography",
        ...
        imageAlt: "bbq pulled pork sandwich, american food, food photography",
        ...
        imageAlt: "greek moussaka layers, mediterranean food, food photography",
        imgCover: "https://images.unsplash.com/photo-1601050690597-df0568f70950?q=80&w=800&auto=format&fit=crop",
        prepTime: "30 min",
        cookTime: "60 min",
        totalTime: 90,
        servings: "4 people",
        ...
        imageAlt: "teriyaki chicken rice bowl, japanese food, food photography",
        imgCover: "https://images.unsplash.com/photo-1546069901-eacef0df6022?q=80&w=800&auto=format&fit=crop",
        prepTime: "15 min",
        cookTime: "20 min",
        totalTime: 35,
        servings: "2 people",
        ...
        imageAlt: "caprese sandwich with pesto, italian sandwich, food photography",
        imgCover: "https://images.unsplash.com/photo-1509722747041-616f39b57569?q=80&w=800&auto=format&fit=crop",
        prepTime: "10 min",
        cookTime: "5 min",
        totalTime: 15,
        servings: "2 people",
...
const app = new RecipeApp(recipes, {
    ...
    carbsValue: '#carbs-value',
    fatValue: '#fat-value',
    fiberValue: '#fiber-value',
